...
from backend.spectral.rovib_cache import (
    load_cached_correction,
    make_rovib_cache_key,
    save_cached_correction,
)
from backend.spectral.rovib_corrections import resolve_alpha_components
...
class OrcaBackend(QuantumBackend):
    ...
    def run_cheap_opt(self, coords_ang: np.ndarray) -> np.ndarray | None:
        ...
        try:
            from backend.orca.orca_cheap_opt import minimize_geometry_cheap_orca  # pylint: disable=import-outside-toplevel
