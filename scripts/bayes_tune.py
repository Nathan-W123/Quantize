...
Examples:
  python scripts/bayes_tune.py --backend orca --n-calls 15
  python scripts/bayes_tune.py --backend orca --orca-exe /path/to/orca --n-calls 15
  python scripts/bayes_tune.py --backend spectral-only --n-calls 20
"""
...
ensure_repo_paths(_ROOT)

from backend.conformers.geometryguess import guess_geometry_molecular_input
from backend.priors.bayes_tune import tune_molecular_optimizer
...
def main():
    ...
    parser.add_argument(
        "--workdir",
        type=str,
        default=str(OUTPUT_TRIALS_DIR),
        help="Root directory for trial workdirs",
    )
    ...
    if float(result.fun) >= 999999:
        print(
            "\nNote: objective stayed at the failure penalty — check ORCA/Psi4 paths, "
            "disk space, and bayes_tune logs under the workdir."
        )
